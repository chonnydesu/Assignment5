...
def create(db: Session, recipe):
    ...
    resource_exists = db.query(models.Resource).filter(models.Resource.id == db_recipe.resource_id).first()

    if not resource_exists:
        print("Resource not found")
        raise ValueError(f"Resource ID {models.Resource.id} does not exist.")

    # CHECK IF THE RESOURCE EXISTS!!
    sandwich_exists = db.query(models.Sandwich).filter(models.Sandwich.id == db_recipe.sandwich_id).first()

    if not sandwich_exists:
        print("Sandwich not found")
        raise ValueError(f"Sandwich ID {models.Sandwich.id} does not exist.")
