...
def create_recipe(recipe: schemas.RecipeCreate, db: Session = Depends(get_db)):
    if create_recipe is None:
        raise HTTPException(status_code=404, detail="Sandwich/Resource not found")
...
def create_orderDetails(orderr: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create(db=db, orderr=orderr)
...
def read_one_orderDetails(order_id: int, db: Session = Depends(get_db)):
    orderr = order_details.read_one(db, order_id=order_id)
    if orderr is None:
...
def update_one_orderDetails(order_id: int, order: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
...
def delete_one_orderDetails(order_id: int, db: Session = Depends(get_db)):
